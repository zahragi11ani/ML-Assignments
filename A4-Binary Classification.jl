...
function get_features(pm::Polynomial3Model, _X::AbstractMatrix)
	...
	for x in 1:(m+1)
		for y in x:(m+1)
			for z in y:(m+1)
				Φ[:,c] .= augmented_X[:,x].*augmented_X[:,y].*augmented_X[:,z]
				c += 1
			end
		end
	end
...
function gradient(lm::AbstractModel, ce::CrossEntropy, X::Matrix, Y::Vector)
	...
	#### BEGIN SOLUTION
	∇w .= sum([(sigmoid(predict(lm, X[i,:])) - Y[i]) * X[i,:] for i in
	1:length(Y)])/length(Y)
	#### END SOLUTION
...
function update!(lm::LinearModel, 
	...
	gbar = opt.gbar
	gbar = β*gbar + ((1 - β).*(g.^2))
	lm.w .= lm.w .- (opt.η./((gbar .+ opt.ϵ).^(1/2))).*g
	opt.gbar = gbar
...
# H₀: µ₁ = µ₂
# H₁: µ₁ > µ₂
# Where µ₂ is the the true expected squared error for PolynomialLogisticRegression and µ₁ is the true expected squared error for LogisticRegression
